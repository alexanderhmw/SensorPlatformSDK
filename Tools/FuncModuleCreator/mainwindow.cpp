...
void MainWindow::on_create_clicked()
{
	...
	{
		{
			output.setFileName(QString("%1/%2_def.h").arg(projectdir).arg(funcmodulename));
		...
		{
			output.setFileName(QString("%1/%2.h").arg(projectdir).arg(funcmodulename));
			output.open(QFile::WriteOnly | QFile::Text);
			output.write(QString("#ifndef %1_H\n#define %1_H\n\n#include \"%2_def.h\"\n\n//Add headers below\n\n#endif").arg(funcmodulename.toUpper()).arg(funcmodulename).toUtf8());
			output.close();
		}
		{
			output.setFileName(QString("%1/%2.cpp").arg(projectdir).arg(funcmodulename));
			output.open(QFile::WriteOnly | QFile::Text);
	...
	for(i=0;i<n;i++)
	{
		...
		for(j=0;j<m;j++)
		{
			output.setFileName(QString("%1/%2_%3.cpp").arg(projectdir).arg(funcmodulename).arg(nodestate[i].nodefunc[j].filesuffix));
			output.open(QFile::WriteOnly | QFile::Text);
		...
		if(nodestate[i].withwidget)
		{
			...
			for(j=0;j<m;j++)
			{
				output.setFileName(QString("%1/%2_%3.cpp").arg(projectdir).arg(funcmodulename).arg(nodestate[i].widgetfunc[j].filesuffix));
				output.open(QFile::WriteOnly | QFile::Text);
